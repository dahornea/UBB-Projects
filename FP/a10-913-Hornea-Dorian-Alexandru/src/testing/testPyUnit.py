...
class DomainAndValidatorsTests(unittest.TestCase):
    def test_create_student(self):
        student = Student(1, "Matthew", 913)
        self.assertEqual(student.get_id_stud(), 1)
        self.assertEqual(student.get_name(), "Matthew")
        self.assertEqual(student.get_group(), 913)
        self.assertEqual(str(student), "1. Matthew from group 913")
        student.set_name("Batman")
        self.assertEqual(student.get_name(), "Batman")
...
class RepoStudentTests(unittest.TestCase):
    def test_student_add_repo(self):
        ...
        repository.add_student(Student(1, "Alex", 913))
        self.assertEqual(len(repository.get_all_students_repo()), 1)
        self.assertEqual(repository.get_all_students_repo(), [Student(1, "Alex", 913)])
        repository.add_student(Student(2, "Dorian", 913))
        self.assertEqual(repository.get_all_students_repo(), [Student(1, "Alex", 913), Student(2, "Dorian", 913)])
        student = Student(1, "Williams", 913)
    ...
    def test_student_remove_repo(self):
        ...
        self.assertEqual(repository.get_all_students_repo(), [Student(2, "Dorian", 913)])
        self.assertRaises(RepositoryError, repository.remove_student_by_id, 1)

    def test_student_update_name_repo(self):
        repository = RepoStudent()
        repository.add_student(Student(1, "Kanye", 913))
        repository.update_by_id_student_name(1, "West")
        self.assertEqual(repository.get_all_students_repo(), [Student(1, "West", 913)])
        self.assertRaises(RepositoryError, repository.update_by_id_student_name, 2, "Kendrick")

    def test_student_update_group_repo(self):
        repository = RepoStudent()
        repository.add_student(Student(1, "Kendrick", 913))
        repository.update_by_id_student_group(1, 911)
        self.assertEqual(repository.get_all_students_repo(), [Student(1, "Kendrick", 911)])
        self.assertRaises(RepositoryError, repository.update_by_id_student_group, 2, 911)

    def test_student_search_by_id_repo(self):
        repository = RepoStudent()
        repository.add_student(Student(1, "Kendrick", 913))
        self.assertEqual(repository.search_by_id_student(1), Student(1, "Kendrick", 913))
...
class ServiceStudentTests(unittest.TestCase):
    ...
    def test_student_add_service(self):
        ...
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Rocky", 914)
        self.assertEqual(len(self.srv_student.get_all_students()), self.srv_student.number_of_students())
        self.assertEqual(self.srv_student.get_all_students(), [Student(1, "Kanye", 913), Student(2, "Rocky", 914)])

    def test_student_remove_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Kendrick", 914)
        self.srv_student.remove_student(1)
        self.assertEqual(self.srv_student.get_all_students(), [Student(2, "Kendrick", 914)])

    def test_student_update_name_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.update_student_name(1, "Kendrick")
        self.assertEqual(self.srv_student.get_all_students(), [Student(1, "Kendrick", 913)])

    def test_student_update_group_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.update_student_group(1, 911)
        self.assertEqual(self.srv_student.get_all_students(), [Student(1, "Kendrick", 911)])
...
class ServiceGradeTests(unittest.TestCase):
    ...
    def test_add_assignment_to_group_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Rocky", 913)
        self.srv_assignment.add_assignment(1, "Assignment678", datetime.date(2020, 11, 11))
    ...
    def test_remove_all_assignments_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Kendrick", 911)
        self.srv_assignment.add_assignment(1, "Assignment678", datetime.date(2020, 11, 11))
    ...
    def test_remove_all_grades_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Rocky", 911)
        self.srv_assignment.add_assignment(1, "Assignment678", datetime.date(2020, 11, 11))
        self.srv_assignment.add_assignment(2, "Assignment678", datetime.date(2020, 12, 12))
        self.srv_grade.add_assignment_to_student(1, 1)
        self.srv_grade.add_assignment_to_student(1, 2)
    ...
    def test_first_statistic_service(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.srv_student.add_student(2, "Rocky", 911)
        self.srv_student.add_student(3, "Kendrick", 916)
    ...
    def test_second_statistic_service(self):
        self.srv_student.add_student(1, "Kendrick", 913)
...
class ServiceUndoTests(unittest.TestCase):
    ...
    def test_undo_redo_add_student(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 913)])
        self.srv_undo.undo_add_student(Student(1, "Kanye", 913))
        self.assertEqual(self.repo_student.get_all_students_repo(), [])
        self.srv_undo.redo_add_student(Student(1, "Kanye", 913))
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 913)])
    ...
    def test_undo_redo_remove_student(self):
        ...
        self.srv_student.remove_student(1)
        self.assertEqual(self.repo_student.get_all_students_repo(), [])
        self.srv_undo.undo_remove_student(Student(1, "Kanye", 913))
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 913)])
        self.srv_undo.redo_remove_student(Student(1, "Kanye", 913))
        self.assertEqual(self.repo_student.get_all_students_repo(), [])

    def test_undo_redo_update_student(self):
        self.srv_student.add_student(1, "Kanye", 913)
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 913)])
        self.srv_student.update_student_name(1, "West")
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "West", 913)])
        self.srv_undo.undo_update_student([1, 1, "Kanye", None])
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 913)])
        self.srv_undo.redo_update_student([1, 1, None, "West"])
        self.srv_undo.undo_update_student([1, 1, "Kanye", None])

        self.srv_student.update_student_group(1, 911)
        self.assertEqual(self.repo_student.get_all_students_repo(), [Student(1, "Kanye", 911)])
